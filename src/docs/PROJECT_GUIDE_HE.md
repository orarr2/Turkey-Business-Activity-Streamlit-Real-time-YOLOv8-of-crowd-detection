...
# oversubscribes וגורמת ל-thrashing של context switches על המכונה הזאת.
...
**למה התיבות גולשות עם התנועה ולא קופאות בין טיקים:** כל תיבה מתפרסמת עם
מהירות ה-track שלה (‏vx,vy בפיקסלים/שנייה, מוחלקת EMA), וכל טיק חתום
בזמן `at` בשעון של הווידאו עצמו (שעון קיר פחות ההיסט מקצה-החי של פלייליסט
ה-HLS, שנמדד מחדש בכל רענון). לולאת הציור בדפדפן (15fps) בוחרת את הטיק
שמתאים ל-`playingDate` של הווידאו ומחצינה כל תיבה לאורך המהירות שלה עם
dt רווי (‏tau=5s) — בין טיקים התיבות ממשיכות בכיוון שנמדד, ובטיק הבא הן
ננעלות שוב על האמת. שערי תצוגה: track מופיע רק אחרי 2 פגיעות רצופות
...
if peak > 0:
    ...
    signal = np.clip(signal + dwell*0.55, 0, 1)           # אזורים ששהתה בהם תנועה נצבעים חם יותר
...
פוליגונים מסוג `parking`: מקום תפוס כשנקודת הרגל של track מאושר ממחלקת
רכב נמצאת בפנים. האירוע נורה רק על **היפוך** תפוס/פנוי — שינוי מצב הוא
מידע, מצב יציב הוא טפט. חישובי התפוסה והשיהוי נעשים פעם אחת לטיק
ומשותפים לרינדור ה-JPEG ולפרסום ה-JSON.
...
על פני פריימים, אף פעם לא פריים בודד שרירותי. זיהוי ו-OCR הם פסקי דין
