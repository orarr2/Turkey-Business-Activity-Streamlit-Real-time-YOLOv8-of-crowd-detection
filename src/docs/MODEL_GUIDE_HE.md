...
בפרויקט זה נבחר המודל `yolov8s`. המשמעות של האות `s` היא "small". משפחת YOLOv8 של Ultralytics כוללת חמישה גדלים סטנדרטיים המסומנים באות אחת: `n` = nano (הקטן ביותר), `s` = small, `m` = medium, `l` = large, `x` = extra-large. ככל שהאות מאוחרת יותר באלף-בית האנגלי, כך המודל גדול יותר, מדויק יותר, אך גם כבד יותר על המעבד.

- **מספר פרמטרים** — משקלים מספריים שהמודל לומד באימון. יותר פרמטרים = יותר קיבולת ייצוג, אבל גם יותר צריכת RAM וזמן חישוב. `yolov8s` מכיל כ-11.2 מיליון פרמטרים ותופס כ-21MB בדיסק.

זהו לא המודל הכי מדויק ולא הכי מהיר — הוא הכי מאוזן למעבד של `e2-micro`.
...
- **‏PP-YOLOE-S** של PaddlePaddle — ביצועים דומים ל-`yolov8s`, קצת קשה יותר לפרוס כי האקוסיסטם קטן יותר. לא שווה החלפה.
...
**המלצה מעשית:** להישאר עם `yolov8s` עוד כ-3-6 חודשים. השדרוג הבא הנכון הוא `yolo11s` (זהה בגודל, mAP+2), לא מודל אחר.
...
**סיכום:** המחברת היא כלי EDA — היא מאפשרת לפרק את הצינור לחלקים ולראות מה כל אחד עושה. שינויים בה **לא משפיעים על הפרודקשן**. אם משהו נראה שגוי במחברת, יש לשקול קודם: האם זה שגיאה במחברת עצמה (`yolov8n` מפסיד ל-`yolov8s`), או שגיאה בקוד הבסיסי (`app/detect_core.py`) — בזה השני הרשמי גם.
...
יש לעדכן את `detect_core.load_model()` כדי לטעון `yolo11s.pt` במקום `yolov8s.pt`. אותו גודל, mAP+2 נקודות על COCO. עדיין דורש lookahead בקוד שאולי מסתמך על שם ספציפי — לא נמצא כזה. הריצה תעבוד out-of-the-box.
...
אותה ארכיטקטורה בדיוק מגיעה בחמישה גדלים, שנבדלים ברוחב וב-עומק השכבות: `n` (nano), `s` (small), `m`, `l`, `x`. ככל שעולים, גדל מספר הפרמטרים וה-**‏FLOPs** (עלות החישוב), עולה הדיוק (mAP), אך גם זמן הריצה וצריכת ה-RAM. זו החלטת ה-trade-off המרכזית של הפרויקט (סעיף 3): `yolov8s` נבחר כי הוא הגדול ביותר שנכנס לחלון הבטוח של `e2-micro` — `yolov8m` הגיע ל-peak של מעל 900MB וגרם ל-oom-kill. המחברת המקומית טוענת דווקא `yolov8n` הקל יותר, כי שם המטרה היא EDA מהירה על CPU של laptop, לא recall מרבי.
...
הקולקטור רץ על `e2-micro` של GCP (1GB RAM, 2 vCPU שיתופיות, בלי GPU) במסלול Always Free. התקציב הזה מגביל את המודל: `yolov8s` הוא הדרג הגדול ביותר שנכנס (yolov8m הגיע ל-peak מעל 900MB ונהרג ב-oom), ו-`imgsz` מוחזק על 512 ב-VM (640 גרם ל-oom-loop אחרי המצלמה החמישית וה-OSNet). מכונה בלי המגבלות האלה (GPU, או אפילו `e2-small` עם 2GB) הייתה מריצה yolov8m/x, imgsz 640/960, או RT-DETR — כולם מדויקים יותר. כלומר מודל הענן אינו הגלאי הכי חזק שקיים, אלא הכי חזק שרץ בחינם 24/7 בתוך 1GB. (המחברת המקומית טוענת דווקא `yolov8n` קל עוד יותר, במכוון, כדי ש-EDA תרוץ מהר על laptop בלי GPU.)
