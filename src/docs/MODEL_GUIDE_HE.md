...
- **במחברת** ניתוח לוקאלי, לרוב על מחשב אישי בלי GPU. `yolov8n` רץ ב-120ms לפריים על CPU של laptop רגיל.
- **ב-VM** רץ קולקטור 24/7 שדורש recall גבוה יותר על אנשים רחוקים. `yolov8s` נותן mAP+7 נקודות במחיר של פי-2 זמן ריצה.

**איך לשנות ל-`yolov8s` במחברת:** יש לשנות את המחרוזת בלבד. אם הקובץ `yolov8s.pt` לא קיים בתיקייה, `ultralytics` יוריד אותו בהרצה הראשונה (~22MB).
