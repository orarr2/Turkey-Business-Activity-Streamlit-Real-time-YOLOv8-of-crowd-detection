...
### תא — בוחר המצלמות הדו-שלבי (code, widget)

הבוחר עובד בשני שלבים כדי לאכוף **מדינה אחת**:

1. **תיבת בחירת מדינה** (Dropdown): טורקיה / תאילנד / יפן / ארה"ב.
2. **תיבות סימון** של מצלמות **אותה מדינה בלבד** — פיזית אי אפשר לערבב מצלמות משתי מדינות, בדיוק כמו שהקולקטור ב-VM רץ (4 מצלמות ממדינה אחת).

בוחרים 1 עד 4 מצלמות ולוחצים **"Apply and continue"**. הבחירה **סופית** לריצה הזו (כדי לשנות: Kernel ‏> Restart Kernel). הפעולה נועלת את הבוחר ומגדירה שלושה משתנים גלובליים:
- `SELECTED_COUNTRY` — המדינה שנבחרה.
- `SELECTED_CAMS` — רשימת מזהי המצלמות.
- `SELECTED_CAMS_APPLIED = True` — הדגל שכל שאר המחברת בודקת.

**מדוע לחיצה אחת מספיקה ל-Run All:** בחירה שהוחלה **שורדת הרצה-מחדש** של תא הבוחר עצמו (לוגיקת `_KEEP`), כך שאחרי לחיצה אחת "Run All" זורם עד הסוף. קרנל טרי תמיד מתחיל ריק ולא-מוחל. ולידציה: 0 מסומנות או יותר מ-4 → שגיאה מפורשת והבחירה לא ננעלת.
