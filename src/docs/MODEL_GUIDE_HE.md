...
המערכת ממירה 4 מצלמות רחוב פומביות בקוניה (טורקיה) לזרם נתונים כמותי. במקום וידאו גולמי, המערכת מפיקה שורות נתונים שאפשר לשאול לגביהן שאלות כמותיות. לפני הפירוט של המדדים, כמה מונחים בסיסיים:
...
- **‏`skyline`** — סטרימינג של `hd-auth.skylinewebcams.com`. **‏HD-Auth / Skyline** הוא ספק של מצלמות רחוב עם עמוד HTML פעיל שמכיל טוקן שמתחלף כל דקה. הקוד גורף את הטוקן מדף ה-HTML כדי לבנות כתובת הזרמה תקפה.
- **‏`webcamera24`** — פורמט מיוחד של הספק `webcamera24.com` שמכיל נגן `tvkur` מוטמע. שולפים את ה-ID של הנגן ובונים playlist מחדש.
...
- **‏`CAM_ID`** — המפתח במילון `CAMERAS`. יש לשנותו כאן אם רוצים מצלמה אחרת. אפשרויות מקדימות בהערה: `giresun_gazi`, `otogar_kavsagi`, `kadikoy`.
...
> אם התקבלה שגיאה `resolve failed`, לרוב זה אחד משלושה מצבים: המצלמה נפלה זמנית (טוקנים של `skyline` לא מתחדשים בשעות מסוימות), הרשת חסומה, או ה-User-Agent נחסם. יש לנסות `CAM_ID = 'konya_hukumet'` שרץ בכל מצב.

---

### תא 7 — כותרת "Single-frame check" (markdown)

מסביר שלפני התחלת איסוף סדרות זמן, כדאי לבדוק שהזרם באמת עובד ושהמודל רואה משהו.
