...
**למה:** המחברת קיימת כדי לענות על שאלה שאי אפשר לענות עליה עם ה-VM לבד: "עד כמה הספירות של ה-VM נכונות?". התאומה שלה (`turkey_business_activity_yolov8s.ipynb`, מפורסמת בריפו מאז 13.08) טוענת את המשקולות של ה-VM כדי לראות בדיוק מה ה-VM היה רואה, כלי אמיתי להשוואת apples-to-apples.
