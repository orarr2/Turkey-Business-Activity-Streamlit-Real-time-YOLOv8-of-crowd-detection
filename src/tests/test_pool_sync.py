...
# ---- reid.db prune helper -----------------------------------------------------

def _seed_reid_db(path: Path, n_entities: int = 3, sightings_per_entity: int = 200,
                  span_days: float = 20.0) -> None:
    """Seed a reid.db shaped like the live one - enough rows for both prune
    knobs to bite (age cutoff AND per-entity cap)."""
    import sqlite3
    ...
    try:
        con.executescript("""
            CREATE TABLE entities (
                ...
                cam_id TEXT NOT NULL, cls TEXT NOT NULL,
                first_seen TEXT NOT NULL, last_seen TEXT NOT NULL,
                sightings INTEGER NOT NULL DEFAULT 1, embedding BLOB NOT NULL);
            CREATE TABLE sightings (
                sighting_id INTEGER PRIMARY KEY AUTOINCREMENT,
                entity_id INTEGER NOT NULL, ts TEXT NOT NULL, similarity REAL);
        """)
        now = time.time()
        for e in range(1, n_entities + 1):
            con.execute("INSERT INTO entities (cam_id, cls, first_seen, last_seen, sightings, embedding) "
                        "VALUES (?, 'car', ?, ?, ?, ?)",
                        (f"cam{e}",
                         time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(now - span_days * 86400)),
                         time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(now)),
                         sightings_per_entity, b"emb"))
            step = (span_days * 86400) / sightings_per_entity
            for k in range(sightings_per_entity):
                ts = time.strftime("%Y-%m-%dT%H:%M:%SZ",
                                   time.gmtime(now - (sightings_per_entity - k) * step))
                con.execute("INSERT INTO sightings (entity_id, ts, similarity) VALUES (?, ?, 0.9)",
                            (e, ts))
        con.commit()
    finally:
        con.close()
...
def test_prune_sightings_drops_old_and_caps_per_entity(tmp_path, monkeypatch):
    """The prune must (a) delete rows past REID_SIGHTINGS_KEEP_DAYS and
    (b) keep at most REID_SIGHTINGS_MAX_PER_ENTITY newest per entity."""
    import sqlite3
    monkeypatch.setattr(pool_sync, "REID_SIGHTINGS_KEEP_DAYS", 7)
    monkeypatch.setattr(pool_sync, "REID_SIGHTINGS_MAX_PER_ENTITY", 20)
    db = tmp_path / "reid.db"
    _seed_reid_db(db, n_entities=3, sightings_per_entity=200, span_days=20.0)
    con = sqlite3.connect(str(db))
    before = con.execute("SELECT COUNT(*) FROM sightings").fetchone()[0]
    con.close()
    assert before == 3 * 200

    n_age, n_cap = pool_sync._prune_sightings_inplace(db)
    con = sqlite3.connect(str(db))
    after = con.execute("SELECT COUNT(*) FROM sightings").fetchone()[0]
    max_per = con.execute(
        "SELECT MAX(c) FROM (SELECT COUNT(*) AS c FROM sightings GROUP BY entity_id)"
    ).fetchone()[0]
    con.close()

    assert n_age > 0, "old rows must be deleted"
    assert n_cap >= 0
    assert max_per <= 20, "cap violated"
    assert after == 3 * 20, "each entity should hold exactly the cap (all 3 entities are still fresh)"


def test_prune_sightings_never_raises_on_missing_db(tmp_path):
    """A missing DB or a schema mismatch must degrade to a no-op, not
    crash the sync pass - the collector's next round retries."""
    missing = tmp_path / "does_not_exist.db"
    n_age, n_cap = pool_sync._prune_sightings_inplace(missing)
    assert (n_age, n_cap) == (0, 0)
